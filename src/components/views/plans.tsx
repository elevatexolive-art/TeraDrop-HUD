...
export function PlansView() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      <header>
        <HudKicker>Plans</HudKicker>
        <h1 className="mt-1 font-display text-2xl font-semibold">Premium catalog</h1>
      </header>
      <ul className="space-y-2">
        {plans.map((plan) => (
          <li key={plan.key}>
            <HudPanel className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="font-medium">
                  {plan.label}{" "}
                  <span className="font-display text-xs tracking-widest text-subtle">{plan.key}</span>
