...
class FileInfo:
    ...
    is_dir: bool = False
    provider: str = ""
    # Kept server-side only. Some Mini App/CDN URLs need the same auth
    # headers that were used while resolving them; these must never reach the
    # browser as part of the public link.
...
def _tag_files(
    ...
    """Attach provider metadata without changing the resolver modules."""
    for file in result.files:
        file.provider = provider
        if headers:
            file.request_headers.update(headers)
    return result
...
async def resolve(share_url: str) -> ResolveResult:
    ...
    if not source.startswith("http") or not detected:
        return ResolveResult(ok=False, message="I could not find a supported link.")

    host_label = str(detected[0]["host"] or "")

    # ---- Dispatch to DiskWala / Flezen / VidBunker / UpFiles resolvers ----
    if host_label == "diskwala":
        from app.resolvers.diskwala import resolve as resolve_diskwala
        result = await resolve_diskwala(source)
        result.host = "diskwala"
        result.surl = detected[0]["surl"]
        ...
        return _tag_files(result, "diskwala", headers)
    if host_label == "flezen":
        from app.resolvers.flezen import resolve as resolve_flezen
        result = await resolve_flezen(source)
        result.host = "flezen"
        result.surl = detected[0]["surl"]
        ...
        return _tag_files(result, "flezen", headers)
    if host_label == "vidbunker":
        from app.resolvers.vidbunker import resolve as resolve_vidbunker
        result = await resolve_vidbunker(source)
        result.host = "vidbunker"
        result.surl = detected[0]["surl"]
        ...
        return _tag_files(result, "vidbunker", headers)
    if host_label == "upfiles":
        from app.resolvers.upfiles import resolve as resolve_upfiles
        result = await resolve_upfiles(source)
        result.host = "upfiles"
        result.surl = detected[0]["surl"]
        return _tag_files(result, "upfiles")

    # ---- Existing TeraBox flow continues below ----
            ...
            try:
                ...
                if parsed.ok:
                    ...
                    if clearance and clearance.cookies:
                        headers["cookie"] = "; ".join(
                            f"{key}={value}" for key, value in clearance.cookies.items()
                        )
                    return _tag_files(parsed, "terabox", headers)
